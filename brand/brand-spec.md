...
### Phone wrapper (dla mockupu/preview)
- Width: 300px, Height: 533px (proporcje 9:16)
- Border-radius: 32px
- Border: `1.5px solid rgba(201,168,76,0.2)`
- Box-shadow: `0 0 60px rgba(0,201,167,0.06), 0 0 120px rgba(0,0,0,0.9)`

---

## 5. Logo Akumuluj

Logo to SVG inline — okrąg z wykresem rosnącym i złotą strzałką.
...
### Elementy logo
- Outer circle: `#07080D` fill + gold gradient stroke
- Inner circle: `#0D0F1A` fill + silver stroke `0.15` opacity
- Chart line: gradient od gold `0.4` do gold `1.0`, dolny do górnego
- Arrow: pełny `#C9A84C`
- Gold compass points: north `0.95`, remaining `0.15` opacity

### Zasada: Logo SVG jest zawsze inline, nigdy jako img src.
...
- **Bez żargonu** — zamiast "aktywa" → "pieniądze", zamiast "portfel akcji" → "inwestycja"
...
Każdy short składa się z 6–8 scen. Nie każda scena musi wystąpić — zależy od tematu.
...
## 8. Animacje — zasady ogólne

> Szczegółowe reguły → `motion-rules.md`

- Każda scena wchodzi animacją — nigdy "snap" bez przejścia
- Elementy wchodzą sekwencyjnie, nie wszystkie naraz
- Typowe wejście: `opacity 0→1` + `translateY(16px)→0` lub `scale(0.9)→1`
- Duration wejścia: 0.4–0.7s
- Easing: `ease`, `cubic-bezier(0.34,1.56,0.64,1)` dla sprężystych elementów
- Transition między scenami: `opacity 0.4s ease`
- Progress bar: liniowa animacja przez cały czas trwania sceny

---

## 9. Synchronizacja z voiceoverem

- HTML czyta `window.AKUMULUJ_TIMING` — array duration w sekundach per scena
- Każda scena przełącza się automatycznie po upływie czasu
- Napisy (subtitles) synchronizowane przez timestamps w ms (`subs` array w każdej scenie)
- Napisy: słowa pojawiają się pojedynczo, kluczowe słowa w złotym kolorze (`hi: true`)

### Format window.AKUMULUJ_TIMING
```javascript
